...
def serve_status(session, client):
    serve_status = ServeStatus(id_client=client.id, time_check_in=datetime.now(UTC), has_been_served=False)
...
def test_client_has_visits(session, client, serve_status):
    visit = Visits(id_client=client.id, id_serve=serve_status.id, date=datetime.now(UTC), was_served=False)
    session.add(visit)
    session.commit()
